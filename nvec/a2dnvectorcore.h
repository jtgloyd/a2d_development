...
//  This SHOULD be does as N templated functions in order to utilize for-loop unwrapping when possible
    template <typename T, class VecType>
    inline T VectorNDotCore(const VecType& x,
                            const VecType& y,
                            const SizeType N) {
        T result = x(0) * y(0);
    ...
    template <typename T, class VecType>
    inline T VectorNNormCore(const VecType& x,
                             const SizeType N) {
