...
//        v(2) *= alpha;
    ...
    template <typename T, class VecType>
    inline T Vec3DotCore(const VecType& x,
                         const VecType& y) {
    ...
    inline T Vec3NormCore(const VecType& x) {
        return sqrt(Vec3DotCore<T, VecType>(x, x));
    }

    template <typename T, class VecType>
    inline void Vec3ScaleCore(const T alpha,
                              const VecType& x,
                              VecType& v) {
        v(0) = alpha * x(0);
        v(1) = alpha * x(1);
    ...
    template <typename T, class VecType>
    inline void Vec3AXPYCore(const T alpha,
                             const VecType& x,
                             const VecType& y,
                             VecType& v) {
        v(0) = alpha * x(0) + y(0);
    ...
    inline void Vec3CrossProductCore(const VecType& x,
                                     const VecType& y,
                                     VecType& v) {
        v(0) = x(1) * y(2) - x(2) * y(1);
        v(1) = x(2) * y(0) - x(0) * y(2);
    ...
    template <class VecType>
    inline void Vec3CrossProductAddCore(const VecType& x,
                                        const VecType& y,
                                        VecType& v) {
        v(0) += x(1) * y(2) - x(2) * y(1);
        v(1) += x(2) * y(0) - x(0) * y(2);
    ...
    inline void Vec3OuterProductCore(const VecType& x,
                                     const VecType& y,
                                     MatType& A) {
        A(0, 0) = x(0) * y(0);
        A(0, 1) = x(0) * y(1);
    ...
    template <class VecType, class MatType>
    inline void Vec3OuterProductAddCore(const VecType& x,
                                        const VecType& y,
                                        MatType& A) {
        A(0, 0) += x(0) * y(0);
        A(0, 1) += x(0) * y(1);
    ...
    template <class VecType, class MatType>
    inline void Mat3x3VecMultCore(const MatType& A,
                                  const VecType& x,
                                  VecType& y) {
    ...
    template <typename T, class VecType, class MatType>
    inline void Mat3x3VecMultAddCore(const MatType& A,
                                     const VecType& x,
                                     VecType& y) {
        y(0) += A(0, 0) * x(0) + A(0, 1) * x(1) + A(0, 2) * x(2);
        y(1) += A(1, 0) * x(0) + A(1, 1) * x(1) + A(1, 2) * x(2);
    ...
    template <class VecType, class MatType>
    inline void MatTrans3x3VecMultCore(const MatType& A,
                                       const VecType& x,
                                       VecType& y) {
    ...
    template <typename T, class VecType, class MatType>
    inline void MatTrans3x3VecMultAddCore(const MatType& A,
                                          const VecType& x,
                                          VecType& y) {
        y(0) += A(0, 0) * x(0) + A(1, 0) * x(1) + A(2, 0) * x(2);
        y(1) += A(0, 1) * x(0) + A(1, 1) * x(1) + A(2, 1) * x(2);
    ...
    template <typename T, class VecType, class MatType>
    inline T Mat3x3InnerProductCore(const MatType& A,
                                    const VecType& x,
                                    const VecType& y) {
